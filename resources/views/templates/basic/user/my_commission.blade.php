    ...
    <div class="padding-top padding-bottom">
        <div class="container">
            <div class="row mb-2">
                ...
                <div class="col-md-12">
                        <form action="{{ route('user.commission.dateSearch')}}"
                                    ...
                                    <tbody>
                                        @forelse($transactions as $trx)
                                            <tr>
                                                ...
                                                <td data-label="@lang('Username')">{{ @$trx->user->id }}</td>
                                                <td data-label="@lang('Amount')" class="budget">
                                                    <strong @if ($trx->trx_type == '+') class="text-success" @else class="text-danger" @endif>
                                                        {{ $trx->trx_type == '+' ? '+' : '-' }}
                                                        {{ getAmount($trx->amount) }} {{ $general->cur_text }}</strong>
                                                </td>
                                                <td data-label="@lang('Charge')" class="budget">
                                                    {{ $general->cur_sym }} {{ getAmount($trx->charge) }} </td>
