        ...
        <div class="row product-details-wrapper">
            ...
            <div class="col-lg-7">
                <div class="product-details-content product-details">
                    ...
                    @php $attr_data = getProductAttributes($product->id, $attr->product_attribute_id); @endphp
                    ...
                    <div>
                        <p>
                            ...
                            @foreach ($product->categories as $category)
                                <a href="{{ route('ecommerce.products.category', ['id'=>$category->id, 'slug'=>slug($category->name)]) }}">{{ __($category->name) }}</a>
                                @if(!$loop->last)
                                /
                                @endif
                            @endforeach
                    ...
                    <div class="product-item-2">
                        <div class="product-item-2-inner wish-buttons-in">
                            ...
                            <div class="product-thumb">
                                <a href="{{route('ecommerce.product.detail', ['id'=>$item->id, 'slug'=>slug($item->name)])}}">
                            ...
                            <div class="product-content">
                                <div class="product-before-content">
                                    <h6 class="title">
                                        <a href="{{route('ecommerce.product.detail', ['id'=>$item->id, 'slug'=>slug($item->name)])}}">{{ $item->name }}</a>
                                    </h6>
                                    <div class="ratings-area justify-content-between">
                                        <div class="ratings">
...
<script>
    ...
    (function($){
        var pid = '{{ $product->id }}';
        load_data(pid);
        function load_data(pid, url="{{ route('ecommerce.product_review.load_more') }}") {
            $.ajax({
                url: url,
                method: "GET",
                data: { pid: pid },
