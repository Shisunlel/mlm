...
<body>
    ...
    <!-- ===========Loader=========== -->
    @include('ecommerce::'.$activeTemplate.'partials.header')
    @if(!request()->routeIs('ecommerce.home') && !request()->routeIs('ecommerce.user.home'))
        @include('ecommerce::'.$activeTemplate .'partials.breadcrumb')
    @endif
    @yield('content')
    @include('ecommerce::'.$activeTemplate.'partials.footer')
    ...
    <script>
        ...
        (function($){
            ...
            $(document).on('click', '.quick-view-btn', function(){
                ...
                $.ajax({
                    url: "{{ route('ecommerce.quick-view') }}",
                    method: "get",
                    data: {
            ...
            */
            ...
            $(document).on('click', '.attribute-btn', function(){
                ...
                if(btn.data('type') == 2 || btn.data('type') == 3){
                    $.ajax({
                        url:"{{ route('ecommerce.product.get-image-by-variant') }}",
                        method:"GET",
                        data:{
                            'id': btn.data('id')
                        },
                ...
                if($(document).find('.attribute-btn.active').length == count_total){
                    ...
                    $(document).find('.attribute-btn.active').each(function(key, attr) {
                        ...
                        if(ti==1){
                            $.ajax({
                                url:"{{ route('ecommerce.product.get-stock-by-variant') }}",
                                method:"GET",
                                data:data,
                                success:function(data)
                                {
            ...
            */
            ...
            $(document).on('click','.cart-add-btn',function(e){
                ...
                $.ajax({
                    ...
                    url:"{{route('ecommerce.add-to-cart')}}",
                    method:"POST",
            ...
            $(document).on('click', '.remove-cart' ,function (e) {
                ...
                var url = '{{route("ecommerce.remove-cart-item", '')}}'+'/'+id;
                $.ajax({
                    headers: {"X-CSRF-TOKEN": "{{ csrf_token() }}"},
                    url: url,
                    method:"POST",
            ...
            $(document).on('click', '.remove-coupon' ,function (e) {
                var btn = $(this);
                var url = '{{route("ecommerce.removeCoupon", '')}}';
                $.ajax({
                    headers: {"X-CSRF-TOKEN": "{{ csrf_token() }}"},
                    url: url,
                    method:"POST",
            ...
            */
            ...
            $(document).on('click','.add-to-wish-list', function(){
                ...
                if($(this).hasClass('active')){
                    ...
                    $.ajax({
                        url: "{{ route('ecommerce.add-to-wishlist') }}",
                        method: "get",
                        data: data,
                        success: function(response){
                            if(response.success) {
            ...
            $(document).on('click', '.remove-wishlist' ,function (e) {
                ...
                var url = '{{route("ecommerce.removeFromWishlist", '')}}'+'/'+id;
            ...
            $(document).on('click','.add-to-compare', function(){
                ...
                if($(this).hasClass('active')){
                    ...
                    $.ajax({
                        url: "{{ route('ecommerce.addToCompare') }}",
                        method: "get",
                        data: data,
                        success: function(response){
                            if(response.success) {
        ...
        function getCompareData() {
            $.ajax({
            url: "{{ route('ecommerce.get-compare-data') }}",
                method: "get",
                success: function(response){
        ...
        function getWishlistData(){
            $.ajax({
                url: "{{ route('ecommerce.get-wishlist-data') }}",
                method: "get",
                success: function(response){
        ...
        function getWishlistTotal(){
            $.ajax({
                url: "{{ route('ecommerce.get-wishlist-total') }}",
                method: "get",
                success: function(response){
        ...
        function getCartTotal(){
            $.ajax({
                url: "{{ route('ecommerce.get-cart-total') }}",
                method: "get",
                success: function(response){
        ...
        function getCartData(){
            $.ajax({
                url: "{{ route('ecommerce.get-cart-data') }}",
                method: "get",
                success: function(response){
    ...
    @stack('script-lib')
        @include('ecommerce::'.$activeTemplate.'partials.notify')
    @stack('script')
</body>
</html>
