            ...
            <div class="compare-table-wrapper">
                ...
                <table class="compare-table">
                    <tbody>
                        ...
                        <tr>
                            @foreach ($compare_items as $item)
                                <td class="product-{{ $item->id }}"><a href="{{route('ecommerce.product.detail', ['id'=>$item->id, 'slug'=>slug($item->name)])}}" class="cmn-btn btn-block">@lang('Buy Now')</a></td>
                            @endforeach
                        </tr>
    ...
    <script>
        ...
        (function($){
            $('.remove-compare').on('click', function(){
                ...
                let className = `.product-${id}`;

                var data = {id:id};
                $.ajax({
                    headers: {"X-CSRF-TOKEN": "{{ csrf_token() }}"},
                    url: "{{route('ecommerce.del-from-compare', '')}}"+"/"+id,
                    method:"post",
                    data: data,
