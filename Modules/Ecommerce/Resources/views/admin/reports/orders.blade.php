        ...
        <div class="card b-radius--10 ">
            <div class="card-body">
                <div class="row justify-content-end mb-2">
                    <div class="col-lg-3">
                        <form action="
                            @if(request()->routeIs('ecommerce.admin.report.order') ||request()->routeIs('ecommerce.admin.report.order.search'))
                                {{ route('ecommerce.admin.report.order.search') }}
                            @else
                                {{ route('ecommerce.admin.report.order.user_search', $user->id) }}
                            @endif
                            " method="GET">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Order ID')" class="font-weight-bold">
                                    <a href="{{ route('ecommerce.admin.order.details', $order->id) }}">{{ strtoupper($order->order_number) }}</a>
                                </td>

                                <td data-label="@lang('Customer')">
                                    <a href="{{ route('ecommerce.admin.users.detail', $order->user->id) }}">{{ $order->user->username }} </a>
                                </td>
            ...
            <div class="card-footer py-4">
                <nav aria-label="...">
                    {{ $orders->appends(['search'=>@$search])->links('ecommerce::admin.partials.paginate') }}
                </nav>
            </div>
        </div>
    </div>
