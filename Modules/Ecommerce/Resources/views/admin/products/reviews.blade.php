...
<div class="row">
    <div class="col-lg-12">
        <div class="card b-radius--10">
            @if(!request()->routeIs('ecommerce.admin.product.reviews.trashed'))
            <div class="card-body">
                    <div class="row justify-content-end">
                        <div class="col-lg-4 mb-3">
                            <form action="{{ route('ecommerce.admin.product.reviews.search') }}" method="GET" >
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Action')">
                                    <button type="button" class="icon-btn mr-1 view-btn" data-toggle="tooltip" title="@lang('View')" data-user="{{ __($review->user->username)}}" data-rating="{{ $review->rating }}" data-review="{{ $review->review }}" data-user_link="{{ route('ecommerce.admin.users.detail', $review->user->id) }}">
            ...
            <div class="card-footer py-4">
                <nav aria-label="...">
                    {{ $reviews->appends(['search'=>request()->search ?? null])->links('ecommerce::admin.partials.paginate') }}
                </nav>
            </div>
...
<script>
    ...
    (function($){
        ...
        form.action = '{{route('ecommerce.admin.product.review.delete', '')}}'+'/'+id;
        modal.modal('show');
    });
...
@push('breadcrumb-plugins')
    @if(!request()->routeIs('ecommerce.admin.product.reviews'))
        @if(request()->routeIs('ecommerce.admin.products.trashed.search'))
        <a href="{{route('ecommerce.admin.product.reviews.trashed')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-backward"></i>@lang('Go Back')</a>
        @else
        <a href="{{route('ecommerce.admin.product.reviews')}}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="las la-backward"></i>@lang('Go Back')</a>
        @endif
    @endif

    @if(request()->routeIs('ecommerce.admin.product.reviews'))
    <a href="{{ route('ecommerce.admin.product.reviews.trashed') }}" class="btn btn-sm btn--danger box--shadow1 text--small"><i class="la la-trash-alt"></i>@lang('Trashed')</a>
    @endif
@endpush
