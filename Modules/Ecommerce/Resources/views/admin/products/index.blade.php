    ...
    <div class="col-lg-12">
        <div class="card b-radius--10">
            <div class="card-body">
                ...
                <div class="row justify-content-end">
                    <div class="col-lg-4 mb-3">
                        <form action="{{ request()->routeIs('ecommerce.admin.products.trashed') || request()->routeIs('ecommerce.admin.products.trashed.search')?route('ecommerce.admin.products.trashed.search'):route('ecommerce.admin.products.search') }}" method="GET" >
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-label="@lang('Name')">
                                    <a href="{{ route('ecommerce.admin.products.edit', [$product->id, slug($product->name)]) }}"><span class="name mb-0"  onclick="{{$product->trashed()?'return false':''}}" data-toggle="tooltip" data-placement="top" title="{{ __($product->name) }}">
                                ...
                                <td data-label="@lang('Action')">
                                    ...
                                    <a href="@if($product->trashed()) javascript:void(0) @else {{ route('ecommerce.admin.products.edit', [$product->id, slug($product->name)]) }} @endif" class="icon-btn btn--primary {{$product->trashed()?'disabled':''}} mr-1" data-toggle="tooltip" data-placement="top" title="@lang('Edit')">
                                        <i class="la  la-edit"></i>
                                    </a>

                                    <a href="@if($product->trashed()) javascript:void(0) @else {{ route('ecommerce.admin.products.attribute-add', [$product->id]) }} @endif" class="icon-btn btn--info text-white {{ $product->trashed()?'disabled':''}} {{ $product->has_variants?'':'disabled' }} mr-1" data-toggle="tooltip" data-placement="top" title="@lang('Add Variants')">
                                        <i class="la la-palette"></i>
                                    </a>

                                    <a href="@if($product->trashed()) javascript:void(0) @else {{ route('ecommerce.admin.products.stock.create', [$product->id]) }} @endif" class="icon-btn btn--warning text-white {{$product->trashed()?'disabled':''}} {{ $product->track_inventory?'':'disabled' }} mr-1" data-toggle="tooltip" data-placement="top" title="@lang('Manage Inventory')">
            ...
            <div class="card-footer py-4">
                <nav aria-label="...">
                    {{ $products->appends(['search'=>request()->search ?? null])->links('ecommerce::admin.partials.paginate') }}
                </nav>
            </div>
...
<div class="modal fade" id="highlight-modal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('ecommerce.admin.products.highlight')}}" method="post">
                @csrf
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('ecommerce.admin.products.all'))
    <a href="{{ route('ecommerce.admin.products.create') }}" title="@lang('Shortcut'): shift+n" class="btn btn-sm btn--success box--shadow1 text--small"><i class="la la-plus"></i>@lang('Add New')</a>
    @else
        @if(request()->routeIs('ecommerce.admin.products.trashed.search'))
        <a href="{{route('ecommerce.admin.products.trashed')}}" class="btn btn-sm btn--primary box--shadow1 text--small">
            <i class="la la-backward"></i>@lang('Go Back')</a>
        @else
        <a href="{{route('ecommerce.admin.products.all')}}" class="btn btn-sm btn--primary box--shadow1 text--small">
            <i class="la la-backward"></i>@lang('Go Back')</a>
        @endif
    @endif

    @if(request()->routeIs('ecommerce.admin.products.all'))
    <a href="{{ route('ecommerce.admin.products.trashed') }}" class="btn btn-sm btn--danger box--shadow1 text--small"><i class="la la-trash-alt"></i>@lang('Trashed')</a>
    @endif
@endpush
...
<script>
    ...
    (function ($) {
        ...
        $(document).keypress(function (e) {
            ...
            if(unicode == 78){
                window.location = "{{ route('ecommerce.admin.products.create') }}";
            }
        });
        ...
        $('.deleteBtn').on('click', function () {
            ...
            form.action = '{{ route("ecommerce.admin.products.delete", '') }}'+'/'+id;
            modal.modal('show');
        });
