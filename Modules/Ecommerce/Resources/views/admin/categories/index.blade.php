...
<div class="row justify-content-center">
                        ...
                        @foreach ($categories as $category)
                            <li class="folder-root @if($category->allSubcategories->count()>0) open @endif" >
                                ...
                                <span type="button" class="p-0 ml-3 delete-btn bg-danger d-none" data-id="{{ $category->id }}"><i class="las la-times-circle"></i></span>
                                    ...
                                    <ul>
                                        @include('ecommerce::admin.categories.subcategory', ['subcategory' => $subcategory])
                                    </ul>
                                    @endforeach
                                </li>
                        @endforeach
    ...
    <div class="col-lg-8">
        <div class="card b-radius--10 ">
            <div class="card-body">
                <form action="{{ route('ecommerce.admin.category-store', 0) }}" method="POST" enctype="multipart/form-data" id="addForm">
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('ecommerce.admin.category.all'))
    ...
    @else
        @if(request()->routeIs('ecommerce.admin.category.trashed.search'))
            <a href="{{ route('ecommerce.admin.category.trashed') }}"
                class="btn btn-sm btn--primary box--shadow1 text--small">
                <i class="la la-fw la-backward"></i> @lang('Go Back')
            </a>
        @else
            <a href="{{ route('ecommerce.admin.category.all') }}"
                class="btn btn-sm btn--primary box--shadow1 text--small">
                <i class="la la-fw la-backward"></i> @lang('Go Back')
            </a>
        @endif
    ...
    @if(request()->routeIs('ecommerce.admin.category.all'))
        <a href="{{ route('ecommerce.admin.category.trashed') }}" class="btn btn-sm btn--danger mb-xl-0 mb-2 box--shadow1 text--small"><i class="las la-trash-alt"></i>@lang('Trashed')</a>
    @endif
@endpush
    ...
    <script>
        ...
        (function($){
            ...
            $(document).on('click', '.parent', function(){
                ...
                var action  = `{{ route('ecommerce.admin.category-store', '') }}/${data.id}`
            ...
            $(document).on('click','.add-chlid' ,function(){
                ...
                var action  = `{{ route('ecommerce.admin.category-store', 0) }}`
                form.attr('action', action);
            ...
            $(document).on('click', '.delete-btn' ,function () {
                ...
                form.action = '{{ route('ecommerce.admin.category.delete', '') }}' + '/' + id;
                modal.modal('show');
            });
