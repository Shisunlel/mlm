...
use Modules\Ecommerce\Entities\AppliedCoupon;
use Modules\Ecommerce\Entities\Cart;
use Modules\Ecommerce\Entities\Coupon;
...
class OrderController extends Controller
{
    public function orders($type)
    {
        ...
        switch ($type) {
            case "incomplete-payment":$p_status = 0;
                break;
            case "processing":$status = [1];
                break;
            case "dispatched":$status = [2];
                break;
            case "completed":$status = [3];
                break;
            case "canceled":$status = [4];
                break;
            case "pending":$status = [0];
                break;
            case "all":$orders = Order::where('user_id', auth()->user()->id)->where('payment_status', '!=', 0)->latest()->paginate(getPaginate());
                break;
        ...
        return view('ecommerce::' . activeTemplate() . 'user.orders.index', compact('page_title', 'orders', 'empty_message', 'type'));
    }

    public function orderDetails($order_number)
    {
        $page_title = 'Order Details';
        $order = Order::where('order_number', $order_number)->where('user_id', auth()->user()->id)->with('appliedCoupon')->first();

        return view('ecommerce::' . activeTemplate() . 'user.orders.details', compact('order', 'page_title'));
    }
    ...
    public function confirmOrder(Request $request, $type)
    {
        ...
        if (isset($request->payment)) {
            ...
            return redirect()->route('ecommerce.home')->withNotify($notify);
        }
