...
use Modules\Ecommerce\Entities\Transaction;
...
class ReportController extends Controller
{
    public function transaction()
    {
        ...
        $transactions = Transaction::where('remark', 'ecommerce')->with('user')->latest()->paginate(getPaginate());
        $empty_message = 'No transactions.';
        return view('ecommerce::admin.reports.transactions', compact('page_title', 'transactions', 'empty_message'));
    }
    ...
    public function transactionSearch(Request $request)
    {
        ...
        return view('ecommerce::admin.reports.transactions', compact('page_title', 'transactions', 'empty_message', 'search'));
    }
    ...
    public function userTransactionSearch(Request $request, $id)
    {
        ...
        return view('ecommerce::admin.reports.transactions', compact('page_title', 'transactions', 'empty_message', 'search', 'user'));
    }

    public function order()
    {
        $page_title = 'Order Logs';
        $orders = Order::where('payment_status', '!=', 0)->with('user')->latest()->paginate(15);

        $empty_message = 'No orders.';
        return view('ecommerce::admin.reports.orders', compact('page_title', 'orders', 'empty_message'));
    }
    ...
    public function orderByUser($id)
    {
        ...
        $orders = Order::where('user_id', $id)->where('payment_status', '!=', 0)->with('user')->paginate(config('constansts.table.default'));

        $empty_message = 'No orders.';
        return view('ecommerce::admin.reports.orders', compact('page_title', 'user', 'orders', 'empty_message'));
    }
    ...
    public function orderSearch(Request $request)
    {
        ...
        if ($key) {
            $orders = Order::where('payment_status', '!=', 0)->with('user')->where('order_number', 'like', "%$key%")->paginate(config('constansts.table.default'));
            ...
            if ($request->date) {
                ...
                if (count($date) == 2) {
                    $start_date = date('Y-m-d H:i:s', strtotime(trim($date[0])));
                    $end_date = date('Y-m-d H:i:s', strtotime(trim($date[1])));
                    $orders = Order::where('payment_status', '!=', 0)->with('user')->whereBetween('created_at', [$start_date, $end_date])->paginate(getPaginate());

                    $page_title = 'Orders between : ' . showDateTime($start_date, 'd M, Y') . ' to ' . showDateTime($end_date, 'd M, Y');
                } else {
                    $start_date = date('Y-m-d', strtotime(trim($date[0])));
                    $orders = Order::where('payment_status', '!=', 0)->with('user')->whereDate('created_at', $start_date)->paginate(getPaginate());
        ...
        return view('ecommerce::admin.reports.orders', compact('page_title', 'orders', 'empty_message', 'key'));
    }
    ...
    public function userOrderSearch(Request $request, $id)
    {
        ...
        if ($key) {
            $orders = Order::where('user_id', $id)
                ...
                ->with('user')->where('order_number', 'like', "%$key%")
            ...
            if (count($date) == 2) {
                ...
                $orders = Order::where('user_id', $id)->where('payment_status', '!=', 0)->with('user')->whereBetween('created_at', [$start_date, $end_date])->paginate(getPaginate());
                ...
                $orders = Order::where('user_id', $id)->where('payment_status', '!=', 0)->with('user')->whereDate('created_at', $start_date)->paginate(getPaginate());
        ...
        return view('ecommerce::admin.reports.orders', compact('page_title', 'user', 'orders', 'empty_message', 'key'));
    }
}
